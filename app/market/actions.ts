...
import { marketKToDb } from "@/lib/market/kPricing";

/** כולל סוחר + משא ומתן */
const MARKET_SELECT_EXT =
  "id, sofer_id, dealer_id, external_sofer_name, style, size_cm, parchment_type, influencer_style, current_progress, asking_price, target_brokerage_price, potential_profit, currency, last_contact_date, negotiation_notes, expected_completion_date, notes, created_at";
  ...
  try {
    ...
    if (v.dealer_id) {
      const { data: d } = await supabase
        ...
        .eq("user_id", user.id)
        .maybeSingle();
      if (!d || d.type !== "Merchant") {
        return { success: false, error: "הסוחר שנבחר לא נמצא או שאינו מסוג סוחר" };
      }
    }
