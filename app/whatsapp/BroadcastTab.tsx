  ...
  const filteredGroups = groupSearch.trim()
    ? groups.filter(
        (g) =>
          ...
          g.wa_chat_id.toLowerCase().includes(groupSearch.toLowerCase())
      )
    : groups;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        console.error("[BroadcastTab] uploadMedia failed:", res.error);
        toast.error(res.error || "העלאה נכשלה");
        setImageFile(null);
        setImageUrl("");
      }
    } catch (err) {
      console.error("[BroadcastTab] uploadMedia exception:", err);
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      <TabsList className="mb-6 rounded-xl bg-slate-100 p-1">
        <TabsTrigger value="compose" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm">
          <RadioIcon className="size-4 ml-1" />
          שידור חדש
        </TabsTrigger>
      ...
      <TabsContent value="compose" className="mt-0 space-y-6">
        <Card className="shadow-sm rounded-xl border-slate-200 bg-white overflow-hidden">
          ...
          <CardContent className="space-y-4">
            ...
            {groups.length > 0 && (
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-700">קבוצות ספציפיות</label>
                <Input
                  placeholder="חיפוש קבוצה..."
                  value={groupSearch}
                  onChange={(e) => setGroupSearch(e.target.value)}
                  className="mb-2 rounded-xl max-w-xs"
                />
                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto rounded-xl border border-slate-100 p-2">
        ...
        <Card className="shadow-sm rounded-xl border-slate-200 bg-white overflow-hidden">
          <CardHeader>
            <div className="flex items-center gap-2">
              <VariableIcon className="w-5 h-5 text-emerald-500" />
              <CardTitle className="text-base font-semibold text-slate-700">הודעה</CardTitle>
            </div>
            <CardDescription>משתנים דינמיים: {VARIABLES.map((v) => `{${v.key}}`).join(", ")}</CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="mb-1.5 block text-sm font-semibold text-slate-700">מק״ט סופר (Ref)</label>
              ...
              <div>
                <label className="mb-1.5 block text-sm font-semibold text-slate-700">הערות פנימיות</label>
                <Input
                  value={internalNotes}
                  onChange={(e) => setInternalNotes(e.target.value)}
                  placeholder="שם סופר, מחיר מוסכם..."
                  className="rounded-xl"
                />
              </div>
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-semibold text-slate-700">טקסט</label>
              <textarea
                ref={textareaRef}
                value={messageText}
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
              <p className="mb-2 text-xs font-semibold text-slate-600">תצוגה מקדימה</p>
              <p className="text-sm text-slate-700 whitespace-pre-wrap min-h-[2rem]">{previewText || "—"}</p>
            </div>
            <div>
      ...
      <TabsContent value="logs" className="mt-0">
        <Card className="shadow-sm rounded-xl border-slate-200 bg-white overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-base font-semibold text-slate-700">היסטוריית שידורים</CardTitle>
              <CardDescription>סטטוס ותגובות API</CardDescription>
            </div>
